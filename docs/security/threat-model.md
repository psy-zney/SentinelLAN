...
| Quá tải / mất mạng | Endpoint nhạy cảm có rate limit; telemetry Agent có hàng đợi bộ nhớ giới hạn và retry idempotent; command/audit lưu PostgreSQL | Hàng đợi Agent mất khi restart, chưa có broker hoặc multi-node SignalR. Cần giám sát dung lượng, restore drill và ingress DDoS ở hạ tầng |
