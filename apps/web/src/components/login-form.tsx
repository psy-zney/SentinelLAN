...
export function LoginForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const session = await new ApiClient().login(String(data.get("organizationCode")), String(data.get("email")), String(data.get("password")));
      router.push(homePathForRole(session.role));
      router.refresh();
    } catch {
      setError("Sign-in failed. Verify the API and development credentials.");
      setBusy(false);
    }
  }
