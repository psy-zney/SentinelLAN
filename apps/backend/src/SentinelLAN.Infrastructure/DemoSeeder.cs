...
public static class DemoSeeder
{
    public static async Task SeedAsync(SentinelDbContext db, IPasswordHasher passwordHasher, Func<string, string?> getSetting, CancellationToken cancellationToken = default)
    {
        ...
        db.Add(organization);
        db.AddRange(
            new User { OrganizationId = organization.Id, Email = getSetting("SENTINELLAN_DEMO_ADMIN_EMAIL") ?? "admin@sentinellan.local", DisplayName = "Demo Admin", Role = Roles.Admin, PasswordHash = passwordHasher.Hash(password) },
            new User { OrganizationId = organization.Id, Email = "technician@sentinellan.local", DisplayName = "Demo Technician", Role = Roles.Technician, PasswordHash = passwordHasher.Hash(password) },
            new User { OrganizationId = organization.Id, Email = "employee@sentinellan.local", DisplayName = "Demo Employee", Role = Roles.Employee, PasswordHash = passwordHasher.Hash(password) });
